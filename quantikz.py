"""
An IPython extension for generating circuit diagrams using LaTeX/quantikz
...
@magics_class
class Quantikz(Magics):

    @cell_magic
    def quantikz(self, line, cell):
        """Generate and display a circuit diagram using LaTeX/quantikz.
        
        Usage:
        
            %quantikz [key1=value1] [key2=value2] ...

            Possible keys and default values are

                filename = ipynb-quantikz-output
                dpi = 100 (for use with format = png)
                options = europeanresistors,americaninductors
                format = svg (svg or png)

        """
        options = {'filename': 'ipynb-quantikz-output',
                   'dpi': '100',
                   'format': 'png'}
...
def load_ipython_extension(ipython):
    ipython.register_magics(Quantikz)
